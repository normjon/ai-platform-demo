...
| **X-Ray Tracing**           | Distributed traces across Lambda invocations. All platform Lambda functions run with `Active` mode tracing — the Lambda service creates a trace segment for every invocation automatically. A platform-level sampling rule (5%, 1 trace/sec guaranteed) and a named service group are provisioned centrally in the platform layer. Each Lambda execution role holds `xray:PutTraceSegments` and `xray:PutTelemetryRecords`. `aws_xray_sdk` is packaged into both Lambda deployment ZIPs (via `requirements.txt` + `null_resource` build step in Terraform). Handlers annotate traces with `Platform` and `Service` keys so traces appear in the platform group's ServiceMap view. The Prompt Vault handler additionally calls `patch_all()` so S3 writes appear as X-Ray subsegments. |
